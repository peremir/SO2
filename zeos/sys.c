...
char* sys_dyn_mem(int num_bytes) 
{
  ...
  int sbrk_page = (int)((DWord)sbrk >> 12);
  int final_page = (int)((DWord)(sbrk-num_bytes)>>12);
  int diff_pages = sbrk_page - final_page;

  if (final_page > TOTAL_PAGES) 
    return NULL;
  
  if (final_page <= PAG_LOG_INIT_CODE) 
    return NULL;

  if (diff_pages >= 0) 
  {
    for (int i = sbrk_page-1; i >= final_page; --i) 
    {
      if (pt[i].bits.present == 0) 
      {
        int frame = alloc_frame();
          if (frame == -1) 
	    return NULL;
        set_ss_pag(pt, i, frame);
      }
    }
  } 
  else 
  {
    for (int i = sbrk_page; i < final_page; ++i) 
    {
      del_ss_pag(pt, i);
    }
  set_cr3(get_DIR(current()));
